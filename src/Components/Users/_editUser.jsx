...
const _editUser = (props) => {
    ...
    const fetchUserData = async () => {
        try {
            const response = await axios.get('http://localhost:3005/users/' + props.userId);
    ...
    const editExistUser = async () => {
        try {
            const response = await axios.put('http://localhost:3005/users/' + props.userId, userInfo);
            if (response) {
                props.setUserEdited();
            }
        }
    ...
    return (
        <div className='user-view _add-view'>
            ...
            <button className='users-btn btn-success' onClick={() => editExistUser()}>Edit User</button>
        </div>
    )
}
